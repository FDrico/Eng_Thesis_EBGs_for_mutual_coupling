...
donde $k(x)$ es el vector de onda, variable en el espacio, de modo que en la estructura periódica, $k(x) = k(x+n p)$, $\phi_1(x) = E(x)$ y $\phi_2(x) = \frac{\partial E(x)}{\partial x} = \phi'_1(x)$. La matriz del sistema es periódica, por lo que cumple las condiciones del teorema de Floquet, de forma que la solución es de la forma
...
Un resultado análogo en física del estado sólido es el teorema de Bloch, que indica que las soluciones a la función de onda de un electrón en un cristal tiene la forma:
...
% RELACION FLOQUET-BLOCH: https://physics.stackexchange.com/questions/155466/floquet-and-blochs-theorems-connection

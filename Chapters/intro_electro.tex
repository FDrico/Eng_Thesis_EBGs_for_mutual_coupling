...
Bajo estas consideraciones, resulta claro que las configuraciones posibles de campo son transversales: Para cualquier campo magnético $\vec{H}(\vec{r}) = \vec{a}e^{j \vec{\beta} \cdot \vec{r}}$, la ecuación de la divergencia del mismo obliga a que $\vec{a} \cdot \vec{\beta} = 0$.
...
donde $\rho_s$ representa la densidad de carga superficial, $\vec{M}_s$ la densidad de corriente magnética en la superficie, $\vec{J}_s$ la densidad de corriente eléctrica, y los campos a uno y otro lado de la interfaz entre los medios se denotan con los subíndices $1$ y $2$.
...
Si se considera una superficie con una impedancia de onda para polarización TM normalizada respecto de $\eta_0$, $Z_s = R_s + j X_s$, independiente del ángulo de incidencia, entonces, al igualar la impedancia de onda a la impedancia de superficie, y teniendo en cuenta las igualdades de la ecuación \ref{eq:impedancia-onda-superficie}, resulta, en el caso TM,\footnote{En general, las componentes $R_s$ y $X_s$ se utilizan primadas ($R'_s$ y $X'_s$), para indicar que no poseen unidades, dado que sus valores son relativos a $\eta_0$. En este caso, por simplicidad, se ignoraron los apóstrofos correspondientes.}
...
Si la reactancia es nula, un valor de $\beta_z$ se obtiene sólo si la resistencia es muy baja o nula ([c] en la figura). Esto significa que sobre una superficie puramente resistiva, sólo habrá propagación de ondas de superficie cuando la resistencia sea inferior a un umbral dependiente de los medios materiales ($\beta_z=0$, [d] en la figura). Cuando ese umbral de resistencia es superado, el valor de la constante de propagación paralela a la interfaz se anula.
...
\begin{align}
	...
	Z_s^{TM} = \frac{Z_2}{\eta_0}\Biggr\rvert_{x=h} = j \frac{\cos \theta_t}{\sqrt{\epsilon_{r_2}}}\tan(\gamma_2 h \cos\theta_t) = j \frac{\cos \theta_t}{\sqrt{\epsilon_{r_2}}}\tan(	\omega\sqrt{\epsilon_{r_2} \mu_0 \epsilon_0} h \cos\theta_t).
\end{align}
...
\begin{align}
	...
	&\implies Z_{2} = \frac{j\omega \mu_0 \cot(\gamma_{x_2} x)}{\gamma_{x_2}}.
\end{align}

Siguiendo la misma metodología que en el caso TM, la impedancia para $x=h$ resulta:

\begin{align}
	Z_s^{TE} = j\frac{\omega \mu_0 \cot(\gamma_2 h \cos \theta_t)}{\gamma_2 \cos\theta_t}.
\end{align}

Vale aclarar, sin embargo, que para las frecuencias de trabajo de esta tesis, esta expresión resulta meramente ilustrativa, dado que no es posible contar con ondas de superficie para frecuencias menores a 25 GHz en FR-4 de 1.6 mm de espesor dispuesto sobre un plano de tierra, como se explicó para la Figura \ref{fig:intersecciones-tm-te}.
...
\subsection{Conjuntos de antenas}
% Ver apunte tesis overleaf

Un grupo de antenas dispuestas geométricamente, y alimentadas de manera tal que conformen un único sistema radiante, conforman un conjunto de antenas. El primer conjunto de antenas usado extensivamente fue la antena Yagi-Uda, inventada en 1926, cuya fase no puede variarse electrónicamente. Recién durante la Segunda Guerra Mundial se inventaron los conjuntos de antena de fase variable, y a partir de 1950, con la invención de los defasadores de ferrita, la modificación de fase completa fue posible \cite{Stutzman:AntennaTheory}.
...
Hay 3 efectos responsables del acoplamiento mutuo en conjuntos de antenas:
...
Si suponemos que la antena no excitada tiene una impedancia de carga $Z^g_j$ sobre sus terminales (y que el generador de esa antena está apagado, $V^g_j=0$), la corriente entrante generada sobre el elemento $j$ debido a la radiación del elemento $i$ será tal que $V_j=-Z^g_j I_j$, por lo que
...
En general, el diseño de una antena \textit{microstrip} se realiza de forma tal que el máximo de radiación se concentre en la dirección normal al parche, aunque también es posible lograr radiación en dirección paralela al mismo, dependiendo de la geometría elegida. El sustrato elegido para la fabricación suele influir en el ancho de banda y la eficiencia: Una mayor permitividad dieléctrica genera un menor ancho de banda y, si se ignora el efecto de ondas de superficie, un menor acoplamiento con elementos cercanos, debido a que los campos se encuentran contenidos en el sustrato.
...
%%%%
El modelo de líneas de transmisión es el más sencillo y el que ofrece una mayor intuición física, ya que usa los conceptos fundamentales de líneas \textit{microstrip}, analizados en la Sección \ref{sec:lineaMicrostrip}. Consiste en considerar a la antena como dos aperturas radiantes de ancho $W$ y altura $h$ separadas una distancia $L$ por una línea de transmisión de impedancia característica conocida $Z_0$, cada una de las cuales puede ser representada, en el modelo, como una conductancia $G$ y una susceptancia $B$, como se indica en la Figura \ref{fig:circuito-equivalente-antena-ms}, que se obtienen del modelo de apertura radiante, que puede consultarse en \cite{Balanis:Theory}.

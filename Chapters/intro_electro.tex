...
Sobre el patrón se distinguen lóbulos, clasificados como principal (en la dirección de mayor radiación) y menores (en otras direcciones). Si una antena es isotrópica, esta distinción no existe, aunque tal antena es físicamente irrealizable. Cuando no existen lóbulos distinguibles en un dado plano, la antena se clasifica como omnidireccional.
%%%%
...
%%%%
Cuando se considera acoplamiento mutuo entre los elementos del \textit{array}, la multiplicación del patrón de radiación por el patrón del arreglo ya no es válida. Los elementos interactúan entre sí y alteran sus corrientes, y por lo tanto sus impedancias. La radiación de una antena induce corrientes sobre las antenas cercanas, que también radían e influyen sobre la corriente de la primer antena. Es un efecto de campo cercano.
...
%%%%
Las antenas \textit{microstrip} cumplen varias de las especificaciones más importantes que imponen las diversas aplicaciones de antenas: Pueden ser pequeñas, tienen bajo peso, son económicas y fáciles de fabricar, y tienen bajo perfil, lo que permite adaptarlas a numerosos diseños. Suelen, sin embargo, tener un alto Q, suelen tener baja eficiencia, son de baja potencia, y presentan polarización impura. La baja eficiencia y el bajo ancho de banda, en muchos casos, suelen mitigarse con el aumento del ancho del sustrato, aunque esto genera ondas de superficie, generalmente indeseadas. Las ondas de superficie extraen potencia que no se convierte en radiación y, facilitan el acoplamiento entre elementos y, cuando inciden sobre discontinuidades, generan lóbulos secundarios que degradan el patrón de radiación y las características de polarización \cite{Balanis:Theory}.
...
Existen varios métodos de análisis de antenas \textit{microstrip}, entre los que se destacan, por su sencillez, el modelo de líneas de transmisión y el modelo de cavidades resonantes, principalmente aplicables a parches rectangulares, que son los de interés en este trabajo.
%%%%
...
%%%%
El acoplamiento mutuo está determinado principalmente por los campos que existen sobre la interfaz dieléctrico-aire: la onda espacial (decrece como $1/\rho$), los campos cercanos de superior (decreciendo como $1/\rho^2$), la onda de superficie (con dependencia de tipo $1/\rho^{1/2}$) y las ondas de tipo \textit{leaky} (con dependencia del tipo $e^{-Q}/e^{1/2}$). A mayor distancia del elemento radiante fuente, las ondas de superficie son las que tienen carácter dominante, siempre que en esa dirección existan \cite{Alexopoulos:PrintedDipoles}. En general, si la distancia entre los elementos radiantes es grande (mayor a $0.8 \lambda$), la influencia de las ondas de superficie es baja y el acoplamiento es por tanto bajo, aunque si el conjunto es grande, la suma de los efectos de los distintos parches puede dar lugar a efectos de acoplamiento significativos, incluso entre elementos no adyacentes \cite{Dodov:SurfaceWavesImpact}.

Utilizando el modelo de cavidad, donde los campos interiores a la estructura Microstrip se desarrollan en sus modos resonantes (requiriendo la solución de un problema de autovalores), y donde los campos radiados se obtienen de la aplicación del teorema de equivalencia, la relación entre los campos y las corrientes queda:

\begin{equation}
M_1 = 2 t E_{z}(x,y) \hat{z} \times \hat{n}, \qquad t=\text{ancho del dieléctrico}
\end{equation}

donde $\hat{n}$ es la normal al límite de la cavidad, y el campo eléctrico $E_z(x,y)$ se calcula a partir de los modos posibles en la cavidad. El problema se reduce a dos aros magnéticos (rectangulares) que radían en la mitad superior del espacio libre, de manera que sólo hay que calcular integrales de línea.

Considerando la expresión de la potencia radiada por las dos antenas, integrando sobre una superficie que encierra a ambas:

\begin{equation}
P_{rad} = \int \frac{\vec{E} \times \vec{H*}}{2} \cdot \hat{n} dS = - \frac{1}{2} \int (\vec{H}^H \cdot \vec{M} + \vec{E}^H \cdot \vec{J*}) dS
\end{equation}

Aplicando el teorema de Gauss, y considerando conductividad perfecta:

\begin{equation}
P_{rad} = \int \frac{\vec{E} \times \vec{H*}}{2} \cdot \hat{n} dS
\end{equation}

La impedancia mutua resulta, excitando el parche 1 y observando el efecto sobre el parche 2:

\begin{equation}
Z_{21}=-\frac{1}{I_1 I_{21}} \int_v \underbrace{\cancel{(E_{21} \cdot J_{2}}}_{\text{sólo fuentes magnéticas}} - H_{21} \cdot M_{2}) dv
\end{equation}

En un \textit{array} planar de antenas se deben considerar la amplitud y la fase de la onda incidente. En el plano H, el acoplamiento mutuo es menor al acoplamiento en el plano E (figura \ref{fig:acomplamientoEyH}, \cite{VanLil:TLModelCoupling}), y el aumento de la distancia entre los parches reduce el acoplamiento \cite{Dodov:SurfaceWavesImpact}. Sobre el plano H, el decrecimiento del parámetro $S_{12}$ es mucho más lento, debido a la presencia de ondas de superficie, comenzando a partir del modo TM, que no posee frecuencia de corte inferior, y aumentando con la aparición de modos TE y TM de orden superior, que aparecen con el aumento de espesor del sustrato y el crecimiento de la constante dieléctrica \cite{Alexopoulos:PrintedDipoles}.

Según \cite{Dodov:SurfaceWavesImpact}, los efectos de onda de superficie son importantes cuando el valor de $\epsilon_r$ del sustrato es alto, y si las pérdidas son bajas, la onda de superficie se mantiene en un valor estable con el aumento de la distancia \cite{Gonzalo:PerformancePBG}. Esto hace que el acoplamiento mutuo entre las antenas sea mayor, porque existe una onda de superficie notoria. Por otro lado, según \cite{Penard:MutualCouplingMicrostrip}, si el ancho del dieléctrico es bajo, las ondas de superficie son despreciables.

Resultados de las mediciones de acoplamientos mutuos se pueden observar en la figura \ref{fig:acoplamientomutuomicrostrip-penard}. A medida que aumenta la distancia entre los parches, el acoplamiento disminuye, mientras que si el valor de t (ancho del dieléctrico) aumenta, el acoplamiento tiende a aumentar ligeramente, posiblemente debido a la aparición de ondas de superficie. Para el mismo arreglo, la autoimpedancia resulta, a mayor distancia entre parches, cada vez más resistiva, decayendo a casi cero su componente imaginaria. Para separaciones pequeñas, los efectos de campo cercano son los que dominan; en la zona intermedia ($0.4\lambda  < d <  0.8 \lambda$), los efectos de la onda espacial directa son visibles, y en la zona más lejana sólo son reconocibles los efectos originados por las ondas de superficie \cite{Alexopoulos:PrintedDipoles}.


Considerando que la impedancia mutua es función de la suma de 3 factores $R_1$, $R_2$ y $R_3$, observar sus variaciones en la esquina inferior izquierda de la figura \ref{fig:acoplamientomutuomicrostrip-penard} puede dar una idea del comportamiento.

Resulta notable, de la figura \ref{fig:acoplamientomutuomicrostrip-penard}, que el acoplamiento depende de la posición relativa entre los elementos. Si los elementos se alinean de forma colineal sobre el plano E, el acoplamiento es mucho mayor que si se ubican de forma colineal sobre el plano H. 
%%%%

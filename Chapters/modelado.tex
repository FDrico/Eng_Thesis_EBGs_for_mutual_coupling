...
Aplicando los mismos pasos que en la sección \ref{sec:diag-de-dispersion}, considerando el teorema de Bloch-Floquet, se obtiene
...
que tiene solución si $|\cos(2\phi + \theta/2)| \geq 1$, lo cual genera soluciones separadas cada $n\pi$, dado que es en esos valores en que el coseno obtiene módulo 1. Esto demuestra que los \textit{bandgaps} producidos se deben principalmente a un efecto de difracción de Bragg, dado que la suma de las fases en cada tramo de las líneas de transmisión que dan lugar a la celda unitaria debe ser múltiplo de $\pi$.

Para finalmente obtener el diagrama de radiación resulta necesario vincular los valores de $\phi$ y $\theta$ con la frecuencia angular $\omega$, a través de la velocidad de propagación del medio, que será distinta para cada tramo de línea.

%%%%%% HASCER UN gRÄFICO

Una segunda forma de obtener el diagrama de radiación consiste en utilizar el análisis de líneas de transmisión generalizadas de la sección \ref{sec_lineas_de_transmision}.

En base al modelo circuital de la figura \ref{fig:circuito-equivalente-kim-parches} se lo analiza como una única línea de transmisión formada por trozos infinitesimales que cumplen con el circuito equivalente propuesto, entonces la impedancia y admitancia por unidad de longitud se pueden expresar como:

\begin{align}
	Z' = j\omega L_p + \frac{\left(j\omega L_w + \frac{1}{j\omega C_g}\right)^2}{2\left( j\omega L_w + \frac{1}{j\omega C_g}\right)} \\
	Y' = j\omega C_p
\end{align}
...
\begin{align}
	\gamma = \sqrt{-ZY} = \sqrt{\omega^2 L_c C_c - \frac{-j\omega^3 L_w^2 C_p}{den}}
\end{align}

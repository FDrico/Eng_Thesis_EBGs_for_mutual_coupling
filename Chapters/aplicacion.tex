...
\begin{figure}[h]
	\centering
	\includegraphics[width=1\textwidth]{Aplicacion/dos-monopolos.png}
...
Se varió la distancia para conocer la variación del coeficiente de acoplamiento con la misma. Los resultados se muestran en la figura \ref{fig:dipolos-distancia-resultados} donde, como resultaba esperable, el mismo disminuye fuertemente con la distancia.
%%% ACA LA FIGURA CORRESPONDIENTE

En vistas de comprender la incidencia del acoplamiento por ondas de superficie sobre el acoplamiento total entre ambas antenas, se eliminó el plano de tierra que se ubica entre ambos dipolos. Esto tiene consecuencias muy notorias sobre el diagrama de radiación, pues deja de existir el plano conductor y, por tanto, la fuente de campo virtual que se consideraba en la figura \ref{fig:monopolos} b) ya no existe. Sin embargo, a medida que aumenta la distancia entre los monopolos y el espacio sin plano de tierra, el comportamiento se parece más al esperado, y se pueden observar los resultados mostrados en la figura \ref{fig:monopolos-sin-plano-de-tierra-resultados}.

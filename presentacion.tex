	...
	\subsection{Ecuaciones de Maxwell} % A subsection can be created just before a set of slides with a common theme to further break down your presentation into chunks
		...
		\subsubsection{Ondas de superficie}
			...
			\begin{columns}[c] % The "c" option specifies centered vertical alignment while the "t" option is used for top vertical alignment
				\column{.55\textwidth}
				\begin{figure}[]
					\includegraphics[width=\textwidth]{intro_electro/ondas-superficie-3d.pdf}
				\end{figure}
				
				\column{.4\textwidth}
				Guías de ondas superficiales:
				\begin{itemize}
			...
			\begin{align*}
				...
				\gamma_z &= \beta_z - j\alpha_z =\sqrt{(\gamma_1^2 - \gamma_{x_1}^2)} = \gamma_1 \sqrt{1+X_s^2 - R_s^2 - 2jR_s X_s}
			\end{align*}
			\end{block}
			...
			\gamma_z = \beta_z-j\alpha_z= \sqrt{(\gamma_1^2 - \gamma_{x_1}^2)} = \frac{\gamma_1}{R_s^2+X_s^2} \sqrt{1+X_s^2 - R_s^2 + 2jR_s X_s}.
			\end{align*}
			\end{block}

...
\authors  {\texorpdfstring
            {\href{fluna@fi.uba.ar}{Federico Luna}}  % Dirección de mail del tesista en la portada de la tesis.
            {Federico Luna}  % Nombre del tesista.
            }
\addresses  {\groupname\\\deptname\\\univname}  % Editar este campo en el archivo "Thesis.cls".
\date{5 de Noviembre de 2018}  % Incluye la fecha al momento de compliar el documento.
\subject{}
\keywords{}
\maketitle
...
Considerando que el tiempo de simulación no resulta despreciable, se propone un modelo circuital de parámetros concentrados, basado en observaciones físicas y experimentales publicadas en la literatura. El circuito propuesto ofrece una respuesta similar a la obtenida mediante la simulación utilizando métodos numéricos de onda completa de la estructura, por lo que resulta una herramienta de interés para el análisis rápido y conceptual del comportamiento de la misma. Además, se propone, implementa y evalúa un método de simulación bidimensional en el dominio del tiempo, basado en la geometría de las estructuras periódicas propuestas.

Finalmente, y a modo introductorio para trabajos futuros, se observa el comportamiento de conjuntos de antenas monopolo y \textit{microstrip} que comparten sustrato, frente a la inserción de geometrías EBG. El breve análisis realizado permite concluir que, si bien la disminución del acoplamiento es notoria, la aparición de desadaptaciones y acoplamientos de campo cercano entre las antenas y la estructura dificulta su uso, en especial si la disposición de celdas unitarias no fue considerada en el diseño original.
...
\chapter{Aspectos de interés en el uso de EBGs en conjuntos de antenas}
...
\printbibheading[title={Bibliografía}]
\nocite{*}
